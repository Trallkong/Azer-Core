...
namespace azer
{
    void SDL3GPURendererSupport::CreateShaders(SDL3GPURenderer* renderer)
    {
        ...
        SDL_GPUShaderCreateInfo skyboxVertInfo {};
        skyboxVertInfo.code        = skybox_vert_spv;
        skyboxVertInfo.code_size   = skybox_vert_spv_len;
        skyboxVertInfo.format      = SDL_GPU_SHADERFORMAT_SPIRV;
        skyboxVertInfo.entrypoint  = "main";
        skyboxVertInfo.stage       = SDL_GPU_SHADERSTAGE_VERTEX;
        skyboxVertInfo.num_uniform_buffers = 1;
        skyboxVertInfo.num_samplers        = 0;
        renderer->m_SkyboxVertexShader = SDL_CreateGPUShader(renderer->m_Device, &skyboxVertInfo);

        SDL_GPUShaderCreateInfo skyboxFragInfo {};
        skyboxFragInfo.code        = skybox_frag_spv;
        skyboxFragInfo.code_size   = skybox_frag_spv_len;
        skyboxFragInfo.format      = SDL_GPU_SHADERFORMAT_SPIRV;
        skyboxFragInfo.entrypoint  = "main";
        skyboxFragInfo.stage       = SDL_GPU_SHADERSTAGE_FRAGMENT;
        skyboxFragInfo.num_uniform_buffers = 0;
        skyboxFragInfo.num_samplers        = 1;
        renderer->m_SkyboxFragmentShader = SDL_CreateGPUShader(renderer->m_Device, &skyboxFragInfo);
    }
    ...
    void SDL3GPURendererSupport::CreateGraphicsPipeline2D(SDL3GPURenderer* renderer)
    {
        ...
        pipeInfo.target_info.depth_stencil_format = renderer->m_DepthTextureFormat;
    ...
    void SDL3GPURendererSupport::CreateGraphicsPipelineSkybox(SDL3GPURenderer* renderer)
    {
        SDL_GPUVertexBufferDescription vtxBufDesc {};
        vtxBufDesc.slot       = 0;
        vtxBufDesc.pitch      = sizeof(SDL3GPURenderer::BatchVertex);
        ...
        SDL_GPUGraphicsPipelineCreateInfo pipeInfo {};
        pipeInfo.vertex_shader    = renderer->m_SkyboxVertexShader;
        pipeInfo.fragment_shader  = renderer->m_SkyboxFragmentShader;
        pipeInfo.vertex_input_state = vtxInput;
        pipeInfo.primitive_type   = SDL_GPU_PRIMITIVETYPE_TRIANGLELIST;
        pipeInfo.rasterizer_state.cull_mode = SDL_GPU_CULLMODE_FRONT;
        ...
        renderer->m_PipelineSkybox = SDL_CreateGPUGraphicsPipeline(renderer->m_Device, &pipeInfo);
    }
    ...
    void SDL3GPURendererSupport::CreateSkyboxSampler(SDL3GPURenderer* renderer)
    {
        SDL_GPUSamplerCreateInfo info {};
        info.min_filter    = SDL_GPU_FILTER_LINEAR;
        info.mag_filter    = SDL_GPU_FILTER_LINEAR;
        ...
        info.address_mode_v = SDL_GPU_SAMPLERADDRESSMODE_CLAMP_TO_EDGE;
        info.address_mode_w = SDL_GPU_SAMPLERADDRESSMODE_CLAMP_TO_EDGE;
        renderer->m_SkyboxSampler = SDL_CreateGPUSampler(renderer->m_Device, &info);
        if (!renderer->m_SkyboxSampler) AZ_CORE_ERROR("Failed to create skybox sampler: {0}", SDL_GetError());
    }
